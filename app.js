...
let devices = []; let currentDeviceId = null; let scanning = false;
let captureToItem = null; let selectedItem = null;
...
async function startCam(){
  await listCameras(); if (!currentDeviceId) return alert("No camera found");
  scanning = true; stopBtn.disabled=false; startBtn.disabled=true;
  codeReader.decodeFromVideoDevice(currentDeviceId, preview, (result, err) => { if (result) onScan(result.getText()); });
}
function stopCam(){ scanning=false; stopBtn.disabled=true; startBtn.disabled=false; codeReader.reset(); }
function flipCam(){
  if (!devices.length) return; const i = devices.findIndex(d=>d.deviceId===currentDeviceId);
  currentDeviceId = devices[(i+1)%devices.length].deviceId; if (scanning){ stopCam(); startCam(); }
}
...
function renderItems(filter=""){
  ...
  const list = items.slice().sort((a,b)=>a.sku.localeCompare(b.sku)).filter(it => !q || it.sku.toLowerCase().includes(q) || it.name.toLowerCase().includes(q));
  itemList.innerHTML = "";
  list.forEach(it => {
...
itemList.addEventListener("click", (e)=>{
  ...
  if (act==="select"){ selectedItem = it; setPanels(); qtyEl.focus(); }
  if (act==="edit"){
    const name = prompt("Name:", it.name); if (name===null) return;
    it.name = (name||"").trim() || it.name;
    const par = prompt("Par (number):", it.par ?? ""); if (par!==null) it.par = Number(par)||0;
    const bc  = prompt("Barcode (optional):", it.barcode || ""); if (bc!==null) it.barcode = (bc||"").trim();
    saveItems();
  }
});
addItemBtn.addEventListener("click", ()=>{
  ...
  items.push({ sku, name, barcode:"", par:0 }); saveItems();
});
...
function setPanels(){
  ...
  const mode = modeEl.value; modePill.textContent = mode==="link" ? "Link mode" : "Count mode";
...
captureBtn.addEventListener("click", ()=>{
  ...
  captureToItem = selectedItem; panelStatus.textContent = "Waiting for scan…";
});
clearBarcodeBtn.addEventListener("click", ()=>{
  if (!selectedItem) return;
  selectedItem.barcode = ""; saveItems(); panelStatus.textContent = "Barcode cleared";
});
addLineBtn.addEventListener("click", ()=>{
  if (!selectedItem) return alert("Select an item");
  const qty = parseInt(qtyEl.value,10); if (!(qty>=0)) return alert("Enter a valid quantity");
  const row = {
    ts: new Date().toISOString(), area: areaEl.value, barcode: selectedItem.barcode || "",
    sku: selectedItem.sku, name: selectedItem.name, qty, user: userEl.value || "", notes: notesEl.value || ""
  };
  lines.push(row); saveLines(); qtyEl.value = ""; notesEl.value = ""; panelStatus.textContent = "Added";
...
function onScan(barcode){
  if (captureToItem){
    ...
    captureToItem.barcode = barcode; saveItems(); panelStatus.textContent = "✔ Linked"; captureToItem = null; return;
  }
  const found = items.find(it => it.barcode===barcode);
  if (found){ selectedItem = found; setPanels(); qtyEl.focus(); }
  else {
    if (!selectedItem){ panelStatus.textContent="Unknown barcode. Select item and link"; alert(`Unknown barcode ${barcode}. Select an item then LINK → Capture.`); return; }
    if (confirm(`Unknown barcode ${barcode}. Link to ${selectedItem.sku}?`)){
      items.forEach(it => { if (it.barcode===barcode) it.barcode=""; });
      selectedItem.barcode = barcode; saveItems(); setPanels();
    }
  }
}

// Lines & Summary with Par highlighting
function renderLines(){
  linesTbody.innerHTML = "";
  lines.forEach((r,i)=>{
    const tr = document.createElement("tr");
    tr.innerHTML = `<td>${new Date(r.ts).toLocaleString()}</td><td>${r.area}</td><td>${r.barcode||""}</td>
      <td>${r.sku}</td><td>${r.name}</td><td>${r.qty}</td><td>${r.user}</td><td>${r.notes}</td>
  ...
  linesTbody.querySelectorAll("button[data-i]").forEach(b=>{
    b.addEventListener("click", e => { const i = +e.target.getAttribute("data-i"); lines.splice(i,1); saveLines(); });
  });
}
function renderSummary(){
  ...
  lines.forEach(r => {
    ...
    cur.total += Number(r.qty)||0; totals.set(r.sku, cur);
  });
  ...
  items.forEach(it => {
    ...
    const tr = document.createElement("tr");
    if (par && tot < par) tr.className = "short"; // highlight shortage row
...
function parseCsv(text){
  ...
  const parseLine = ln => { const cells=[]; let cur="", q=false;
  ...
  for (let r=1;r<lines.length;r++){ const vals=parseLine(lines[r]); const row={}; header.forEach((h,i)=>row[h]=vals[i]); out.push(row); }
  return out;
}
...
exportLinesBtn.addEventListener("click", ()=>{
  ...
  const a = document.createElement("a"); a.href = url; a.download = `counts_${new Date().toISOString().slice(0,19).replace(/[:T]/g,'-')}.csv`; a.click();
  URL.revokeObjectURL(url);
});

// Excel export via ExcelJS with light styling
exportXlsxBtn.addEventListener("click", async ()=>{
  ...
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a"); a.href = url; a.download = `stock_counts_${new Date().toISOString().slice(0,10)}.xlsx`; a.click();
  URL.revokeObjectURL(url);
});
...
exportItemsBtn.addEventListener("click", ()=>{
  ...
  const blob = new Blob([csv], { type:"text/csv;charset=utf-8" });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a"); a.href = url; a.download = `items_${new Date().toISOString().slice(0,19).replace(/[:T]/g,'-')}.csv`; a.click();
  URL.revokeObjectURL(url);
});
importItemsInput.addEventListener("change", async (e)=>{
  ...
  items = next; saveItems(); alert(`Imported ${items.length} items.`); importItemsInput.value = "";
});

// Sync (optional Google Sheets Apps Script)
function renderSyncStatus(){
  const online = !!(cfg.itemsUrl || cfg.countsUrl);
  syncStatusEl.textContent = online ? "Online (Sheets sync configured)" : "Offline (local)";
  itemsUrlEl.value = cfg.itemsUrl; countsUrlEl.value = cfg.countsUrl;
}
...
async function syncAll(){
  ...
  }catch(e){ panelStatus.textContent = "Sync failed"; }
}
